<div>
    <div class="saludos border-b-2 border-solid border-gray-300 ">
        ...
        <p class="text-gray-500 text-lg ">Aqui encontrarás las respuestas de los expertos</p>
    ...
    <div x-data="{ smodal: false }" class=" alerts dbody block  xl:w-4/5 xl:mx-auto ">
        ...
        @foreach ($notifications as $notification)
            <div class=" alerts--notification min  border-l-4 border-main-yellow flex rounded-lg shadow-lg py-2">
                ...
                <div class="notification w-full mx-4 flex flex-col justify-between">
                    ...
                    <!-- <livewire:get-expertforcard :infomsj="$notification"> -->
                    <div class=" flex items-center mb-4 ">
                        <img class="expert__avatar w-8 h-8 xl:w-10 xl:h-10 2xl:w-14 2xl:h-14 rounded-full m-auto lg:m-0" src="{{asset('storage/' . $notification->data['expert_picture']) }}" alt="">
                        <p class="text-lg font-bold ml-4">{{ $notification->data['expert_name'] }}, escribió:</p>
                    </div>               
                    <p class="text-base mb-2 ">{{ $notification->data['expert_msg'] }}</p>                    
                    ...
                    <button 
                        wire:click="selectSender({{ $notification->data['expert_id'] }})"
        ...
        <div x-show="smodal" @click.away="smodal = false" class=" w-1/3 top-60 left-1/3 z-50 border-solid border-4 border-black fixed">
            <div class="modal--header bg-black pl-4 py-3">
                <p class="text-gray-200 font-bold text-lg">Respondiendo a {{ $expert_name }} </p>
            </div>
