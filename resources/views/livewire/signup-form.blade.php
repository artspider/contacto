<div>
  <div  class="registro container  mb-2 mx-auto" id="registro">
    <div class="registro--header flex justify-start mb-6 pl-4">
      <svg class="w-8 h-12 lg:w-12 lg:h-16 " xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 246 353">
        <image id="Capa_1" data-name="Capa 1"  xlink:href="data:img/png;base64,"/>
      </svg>
      <p class="text-black text-center text-3xl xl:text-5xl font-bold ml-2">Registro</p>
    </div>

    <div class="registro--seleccion bg-white rounded-lg shadow-lg w-11/12 lg:w-1/2 max-w-screen-xl mb-6 mx-auto py-2">
      <p class="text-xl font-bold mt-6 mb-4 ml-8">
        Registra tu datos y consigue una cuenta gratis
      </p>

      {{-- Inicia el Formulario --}}
      <form wire:submit.prevent="submit" class="my-4" >
        <div x-data="{ type: 1 }">
          <div wire:model="type" class="mt-6 w-11/12 flex justify-evenly mx-auto">
            <a
              @click="type = 1;$dispatch('input', '1')"
              class="btn--registro btn   w-1/2 rounded-md text-xs md:text-sm tracking-tight text-center font-bold py-6 mr-1"
              :class="{ 'bg-black text-white': type==1, 'bg-main-yellow text-black': type==2 }"
            >
              Busco profesionista
            </a>
            <a
              @click="type = 2; $dispatch('input', '2')"
              class="btn--registro btn  w-1/2 rounded-md text-sm tracking-tight text-center font-bold py-6 ml-1"
              :class="{ 'bg-main-yellow text-black': type==1, 'bg-black text-white': type==2 }"
            >
              Soy un experto
            </a>
          </div>
        {{-- <div class="tipo-cuenta flex justify-center">
          <input wire:model="type" type="radio" value="1" id="employer" name="tipoCuenta">
          <label class="radio text-sm pt-1" for="employer"> Busco profesionista </label>

          <input wire:model="type" type="radio" value="2" id="expert" name="tipoCuenta" >
          <label class="radio text-sm pt-1" for="expert"> Soy un experto </label>
        </div> --}}

        <p class=" text-xs text-red-900 text-center ">
          {{ $errors->first('type') }}
        </p>

        <div class="datos flex flex-col items-center">
          {{-- Email --}}
          <span class="flex justify-center items-center w-full my-2">
            <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="email"><i class="icon-envelope "></i></label>
            <input wire:model="email" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2" type="text" name="email" id="email" placeholder="Email" required/>
          </span>
          <p class=" text-xs text-red-900 ">
            {{ $errors->first('email') }}
          </p>
          {{-- name --}}
          <span class="flex justify-center items-center w-full my-2">
            <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="name"><i class="icon-user"></i></label>
            <input wire:model="name" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2" type="text" name="name" id="name" placeholder="Name" required/>
          </span>
          <p class=" text-xs text-red-900 ">{{ $errors->first('name') }}</p>
          {{-- password --}}
          <span class="flex justify-center items-center w-full my-2">
            <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="password"><i class="icon-shield"></i></label>
            <input wire:model="password" class="w-64 lg:w-1/2 h-10  bg-red-step  rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2"  type="password" name="password" id="password" placeholder="Password" required/>
          </span>

          <span class="flex justify-center items-center w-full my-2">
            <label class="w-12 h-10 text-center bg-main-yellow rounded-l-lg shadow-lg border border-solid border-gray-500 py-2" id="icon" for="password"><i class="icon-shield"></i></label>
            <input wire:model="password_confirmation" class="w-64 lg:w-1/2 h-10             bg-red-step    rounded-r-lg shadow-lg border border-solid border-gray-500 pl-2"  type="password" name="password_confirmation" id="password_confirmation" placeholder="Confirmar password" required/>
          </span>
          <p class=" text-xs text-red-900 ">
            {{ $errors->first('password') }}
          </p>

        </div>
        <span class="flex flex-col justify-center items-center w-11/12 mx-auto my-2">
          <p class=" lg:w-3/5 md:w-1/2 text-sm ml-4 leading-5 font-semibold mb-2">
            Al hacer clic, indicas que estas de acuerdo con nuestros <a class=" text-gray-400 " href="#">terminos y condiciones</a>.
          </p>
          {{-- <a href="#" class="btn btn--primary rounded-md w-full text-sm lg:text-base text-center font-bold px-10 py-4 ">Registrar</a> --}}
          <button type="submit"  class="btn btn--primary rounded-md w-full text-sm lg:text-base text-center font-bold px-10 py-6 " >
            Registrar
          </button>
        </span>
      </div>
      </form>

        {{-- Termina el Formulario --}}

    </div>

    @if ($message = Session::get('message'))
      <div class="alert text-green-100 bg-green-600 mx-auto">
        <button type="button" class="close" data-dismiss="alert">×</button>
        <strong>{{ $message }}</strong>
      </div>
    @endif
      <hr>
  </div>
</div>
